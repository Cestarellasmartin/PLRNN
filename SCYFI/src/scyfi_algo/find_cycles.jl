...
function find_cycles(
    ...
    println(PLRNN==ClippedShallowPLRNN(),get_pool_from_traj)
    if get_pool_from_traj
        if PLRNN==ClippedShallowPLRNN()
            println("shallow")
            relu_pool=construct_relu_matrix_pool_traj_shallow(A, W₁, W₂, h₁, h₂, size(A)[1], size(h₂)[1], num_trajectories, len_trajectories,true)
