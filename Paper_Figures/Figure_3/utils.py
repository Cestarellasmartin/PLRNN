...
def create_savepath(args):
    save_path = os.path.join(args.base_path, args.experiment)
    make_dir(save_path)
    trial_path = os.path.join(save_path, args.name)
    make_dir(trial_path)
    if args.run is None:
...
def load_dataset(args):
    ...
    ## take last trial as test-set ##
    #test_data = data[-1]
    #test_inputs = inputs[-1]

    #data_set = dataset.GeneralDataset(data, inputs, args.seq_len, args.batch_size, args.batches_per_epoch)## drop_last einfügen
    ##args.dim_x = data.shape[1]
    if not isinstance(data, list):
        data_set = dataset.GeneralDataset(data, inputs, args.seq_len, args.batch_size, args.batches_per_epoch)  ## drop_last einfügen

        args.dim_x = data.shape[1]
        args.dim_s = inputs.shape[1]
    else:
        if args.test:
            data_set = dataset.TrialDataset(data[:-1], inputs[:-1], args.seq_len, args.batch_size,
                                            args.batches_per_epoch, args.W_trial)
            args.n_trials = len(data) - 1
        else:
            # data_set = dataset.TrialDataset(data, inputs, args.seq_len, args.batch_size,
            # args.batches_per_epoch, args.W_trial)
            args.n_trials = len(data)
            data_set = dataset.TrialDataset(data, inputs, args.seq_len, args.batch_size, args.batches_per_epoch,
                                            args.W_trial)

        args.dim_x = data[0].shape[1]

        #data_set = dataset.TrialDataset(data[:-1], inputs[:-1], args.seq_len, args.batch_size, args.batches_per_epoch, args.W_trial)
        #data_set = dataset.custom_TrialDataset(data[:-1], inputs[:-1], args.seq_len, args.batch_size, args.batches_per_epoch, args.W_trial)
...
def load_testset(args):
    ...
    inputs = read_data(args.inputs_path)
    if args.test:
        test_set = dataset.TestDataset(data[-1], inputs[-1])
        #args.dim_x = data[-1].shape[1]
        #args.dim_s = inputs[-1].shape[1]
    else:
        test_set = None
...
#def load_inputs(args):
    #inputs = read_inputs(args.inputs_path)
    #inputs_set = dataset.GeneralInputset(inputs, args.seq_len,args.batch_size,args.batches_per_epoch)
    #args.dim_s = inputs.shape[1]
    #return args, inputs_set
