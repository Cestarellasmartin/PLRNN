...
class Regularizer:
    def __init__(self, args):
        ...
        self.a_block = args.A_reg
        self.C_reg = args.C_reg
        self.h_trial = args.h_trial
    ...
    def loss_regularized_parameter(self, parameter, to_value, weighting_of_states):
        ...
        loss = weighting_of_states * self.norm(diff)**2
        # loss = weighting_of_states * self.norm(diff)
        return loss

    # def loss_regularized_parameter_W_trial_old(self, parameter, weighting_of_states, weighting_of_states_lambda2, weighting_of_states_lambda3):
    #     W = parameter
    #     n, M1, M2 = W.size()
    #     W_hat  = W
    ...
    def loss_regularized_parameter_W_trial(self, parameter, weighting_of_states, weighting_of_states_lambda2, weighting_of_states_lambda3):
        ...
        l2_reg = 1 / (2*n) * weighting_of_states[0, 0] * (tc.norm(W_hat, p=2, dim=(1,2))**2).sum()
        term_1st_order = W_hat - W1_hat
        loss_1st_order = 1 / (2*n) * weighting_of_states_lambda2[0, 0] * (tc.norm(term_1st_order, p=2, dim=(1,2))**2).sum()
        term_2nd_order = W_hat - 2*W1_hat + W2_hat
        term_2nd_order[1, :, :] = 0
        loss_2nd_order = 1 / (2*n) *  weighting_of_states_lambda3[0, 0] * (tc.norm(term_2nd_order, p=2, dim=(1,2))**2).sum()

        return l2_reg, loss_1st_order, loss_2nd_order
    
    def loss_regularized_parameter_W_trial_3(self, parameter, weighting_of_states, weighting_of_states_lambda2, weighting_of_states_lambda3):
        W = parameter
        n, M1, M2 = W.size()
        W_hat  = W
        indices1 = [0] + list(range(n-1))
        ...
        term_1st_order = W_hat - W1_hat
        loss_1st_order = 1 / (2*n) * weighting_of_states_lambda2[0, 0] * (tc.norm(term_1st_order, p=2, dim=(1,2))**2).sum()
        term_mean = (tc.norm(W-W.mean(0), p=2, dim=(1,2))**2).sum()
        loss_mean = 1 / (2*n) * term_mean
        return l2_reg, loss_1st_order, loss_mean
    
    def loss_regularized_parameter_h_trial(self, parameter, weighting_of_states, weighting_of_states_lambda2, weighting_of_states_lambda3):
        h = parameter
        n, M1 = h.size()
        h_hat  = h
        indices1 = [0] + list(range(n-1))
        ...
        l2_reg = 1 / (2*n) * weighting_of_states[0, 0] * (tc.norm(h_hat, p=2, dim=(1,2))**2).sum()
        term_1st_order = h_hat - h1_hat
        loss_1st_order = 1 / (2*n) * weighting_of_states_lambda2[0, 0] * (tc.norm(term_1st_order, p=2, dim=(1,2))**2).sum()
        term_2nd_order = h_hat - 2*h1_hat + h2_hat
        term_2nd_order[1, :] = 0
        loss_2nd_order = 1 / (2*n) *  weighting_of_states_lambda3[0, 0] * (tc.norm(term_2nd_order, p=2, dim=(1,2))**2).sum()
    ...
    def loss(self, parameters):
        ...
        if W1.dim() == 2:
            if self.a_block:
                loss += tc.sum(tc.maximum(A, tc.tensor(1.0)) - 1.0)*100 # A-block
            ...
            if self.h_trial:
                terms = self.loss_regularized_parameter_h_trial(parameter=h1, weighting_of_states=self.lambda1_W.view(-1, 1),
            ...
            if self.C_reg:
                loss += self.loss_regularized_parameter(parameter=C, to_value=0., weighting_of_states=0.5)
            else:
            ...
            if self.a_block:
                loss += tc.sum(tc.maximum(A, tc.tensor(1.0)) - 1.0)*100 # A-block
            loss += self.loss_regularized_parameter(parameter=A, to_value=1.0, weighting_of_states=self.lambda1[0]/ (2*n)) #!!!!!!!!!!!!!

            if self.reg_type == 3:
                terms = self.loss_regularized_parameter_W_trial_3(parameter=W1, weighting_of_states=self.lambda1_W.view(-1, 1),
                                                                weighting_of_states_lambda2=self.lambda2.view(-1, 1),
                                                                weighting_of_states_lambda3=self.lambda3.view(-1, 1))
                loss += sum(terms)
                terms = self.loss_regularized_parameter_W_trial_3(parameter=W2, weighting_of_states=self.lambda1_W.view(-1, 1),
                                                                weighting_of_states_lambda2=self.lambda2.view(-1, 1),
                                                                weighting_of_states_lambda3=self.lambda3.view(-1, 1))
                loss += sum(terms)
