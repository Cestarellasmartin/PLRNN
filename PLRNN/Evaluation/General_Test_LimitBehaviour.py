...
def Test_limitPSE(mpath,num_epochs,NeuronPattern,RealData,TimeLength):
    m = Model()
    ...
    Length_trial = round(RealData.shape[0]/len(NeuronPattern["Training_Neuron"]))
    Input_channels= NeuronPattern["Training_Input"][0].shape[1]
    ...
    for i in range(len(NeuronPattern["Training_Neuron"])):
        pse,pse_list = ps.power_spectrum_error(ModelT[i], RealData)
    ...
    for i in range(len(NeuronPattern["Training_Neuron"])):
        ...
        for j in range(Dim_kl):
            kl_dim[j] = kl.calc_kl_from_data(ModelT[i][:,neu_list],
                                              tc.tensor(RealData[:,neu_list]))
            neu_list += 3
        val[i]=kl_dim.mean()
    ...
    pse_ns = []
    pse_nslist = []
    pse_ns,pse_nslist = ps.power_spectrum_error(NS_Signal, RealData[0:NS_Signal.shape[0],:])
...
for i in tqdm(model_list):
    ...
    for j in tqdm(runs):
        ...
        PSE_SS,PSE_NS,KLd=Test_limitPSE(mpath,num_epochs,NeuronPattern,Metadata["BeforeActivity"],TimeLength)
